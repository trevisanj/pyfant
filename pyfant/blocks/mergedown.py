...
class UseNumPyFunc(MergeDownBlock):
...
class SNR(MergeDownBlock):
    ...
    def _do_use(self, inp):
        if self.continua is None:
            continua = splistblocks.UseSpectrumBlock(spblocks.Rubberband(True)).use(inp)
        else:
            continua = self.continua
        cont_2 = splistblocks.UseSpectrumBlock(spblocks.ElementWise(np.square)).use(continua)  # continua squared
