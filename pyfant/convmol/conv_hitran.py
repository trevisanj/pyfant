    ...
    def _make_sols(self, hapidata):
        ...
        for i in range(n):
            try:
                ...
                vl = int(s_vl[i])  # TODO check if already int
                v2l = int(s_v2l[i])
                Jl = (gp[i]-1.)/2.  # Taken from Plez's conversor [2]
                J2l = (gpp[i]-1.)/2.  # TODO double-check which SJ goes, if rounded or with the fucking 0.5
        ...
        a99.get_python_logger().info(f"ConvHitran: in: {cnt_in}/{n}; out: {cnt_out}/{n}")
