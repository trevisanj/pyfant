...
from hypydrive import WDBRegistry
...
class WDBState(WDBRegistry):
    ...
    def _populate(self, restore_mode=None):
        ...
        try:
            ...
            t = self.tableWidget
            rows = hpd.cursor_to_rows(self._f.query_state(id_molecule=self._id_molecule))
    ...
    def _get_edit_params(self):
        ...
        ti = self._f.get_table_info("state")
        params = hpd.table_info_to_parameters(ti)
    ...
    def _do_on_insert(self):
        params = self._get_edit_params()
        form = hpd.XParametersEditor(specs=params, title="Insert Molecular State")
        r = form.exec_()
    ...
    def _do_on_edit(self):
        r, form = hpd.show_edit_form(self.row, self._get_edit_field_names(), "Edit Molecular State")
        if r == QDialog.Accepted:
            kwargs = form.get_kwargs()
            id_ = self.row["id"]
