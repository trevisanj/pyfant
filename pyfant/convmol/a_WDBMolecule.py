...
class WDBMolecule(WDBRegistry):
    ...
    def _populate(self, restore_mode=None):
        ...
        try:
            ...
            rows = hpd.cursor_to_rows(self._f.query_molecule())
    ...
    def _do_on_insert(self):
        params = self._get_edit_params()
        form = hpd.XParametersEditor(specs=params, title="Insert molecule")
        r = form.exec_()
    ...
    def _do_on_edit(self):
        r, form = hpd.show_edit_form(self.row, self._get_edit_field_names(), "Edit molecule")
        if r == QDialog.Accepted:
            kwargs = form.get_kwargs()
