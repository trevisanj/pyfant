...
# __fileobj = None
# def get_conn():
#     return hpd.get_conn(__ALIAS)


class FileHitranDB(hpd.FileSQLiteDB):
    ...
    def _populate(self):
        ...
        try:
            ...
            hpd.get_python_logger().info("Inserted {} molecules".format(len(mols)))
            ...
            for mol in mols:
                ...
                for iso in isos:
                    try:
                        ...
                        hpd.get_python_logger().exception("Tried to insert this: {}".format(iso))
                        raise

                hpd.get_python_logger().info("Inserted {} isotopologues for molecule '{}' ({})".
    ...
    def print_isotopologues(self, **kwargs):
        ...
        data, header = hpd.cursor_to_data_header(r)
