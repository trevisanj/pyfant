...
def run_parallel(rr, max_simultaneous=None, flag_console=True, runnable_manager=None):
    ...
    hpd.get_python_logger().info(
        ("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" + (" ALIVE" if rm.is_alive() else " DEAD")))
    hpd.get_python_logger().info("test-tm2 [SUPPOSED TO HAVE] EXITED")
    hpd.get_python_logger().info(rm)
...
def create_or_replace_or_skip_links(ff, dest_dir="."):
    ...
    for f in ff:
        ...
        if os.path.isfile(ptd) and not os.path.islink(ptd):
            hpd.print_skipped("file exists in local directory")
            flag_skip = True
        else:
            obj = hpd.load_with_classes(f, [pf.FileMain, pf.FileAbonds, pf.FileDissoc, pf.FileToH])
            if obj is not None:
                hpd.print_skipped("detected type %s" % obj.__class__.__name__)
                flag_skip = True
            else:
                obj = hpd.load_with_classes(f, [pf.FileModBin])
                if obj is not None:
                    if len(obj) == 1:
                        hpd.print_skipped("%s of only one record" % obj.__class__.__name__)
                        flag_skip = True
        ...
        if not flag_skip:
            try:
                if os.path.islink(ptd):
                    ...
                    s_action = "created"
                hpd.create_symlink(f, ptd)
                print(("   ... %s link" % s_action))
            except Exception as e:
                hpd.print_error("Error creating link: %s" % str(e))
...
def copy_or_skip_files(ff, dest_dir="."):
    ...
    for f in ff:
        ...
        if os.path.isfile(name):
            ...
            obj = hpd.load_with_classes(f, [pf.FileMain, pf.FileAbonds, pf.FileDissoc])
            if obj is not None:
                pass
            else:
                hpd.print_skipped("neither main, abonds, nor dissoc file")
                flag_skip = True
        ...
        if not flag_skip:
            try:
                ...
                hpd.print_error("Error copying file: %s" % str(e))
