...
class _Option(hpd.AttrsPart):
    ...
    def get_label_text(self):
        return hpd.enc_name_descr("--%s" % self.name, self.short_descr, self.color)
...
class WOptionsEditor(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.logger = hpd.get_python_logger()
        ...
        # x.setWordWrap(True)
        x.setStyleSheet("QTextEdit {color: %s}" % hpd.COLOR_DESCR)
        lu.addWidget(x)

        x = self.labelError = QLabel(self)
        x.setStyleSheet("QLabel {color: %s}" % hpd.COLOR_ERROR)
        lu.addWidget(x)
    ...
    def eventFilter(self, obj_focused, event):
        if event.type() == QEvent.FocusIn:
            ...
            if option:
                text = "%s<br><br>%s" % \
                       (hpd.enc_name(option.name.replace("&", ""), option.color),
                        option.long_descr)
    ...
    def on_preview(self):
        ...
        w = hpd.XText(self, line, "Command line")
        w.show()
    ...
    def __update_data(self):
        ...
        try:
            ...
            if ss:
                ...
                emsg = str(E)
            hpd.get_python_logger().exception("Updating Options object")
    ...
    def __update_error_label(self):
        ...
        if self.error_text:
            l.append('<span style="color: %s"><b>Error</b>: %s</span>' %
                     (hpd.COLOR_ERROR, self.error_text))
        if self.hiding_text:
            l.append('<span style="color: %s"><b>Warning</b>: %s</span>' %
                     (hpd.COLOR_WARNING, self.hiding_text))
        s = "; ".join(l)
