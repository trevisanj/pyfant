...
class XRunnableManager(QMainWindow):
    ...
    def __init__(self, parent=None, rm=None):
        ...
        self.changed_proxy = hpd.SignalProxy(signals,
    ...
    def eventFilter(self, obj, event):
        if obj == self.tableWidget:
            return hpd.check_return_space(event, self.on_tableWidget_cellDoubleClicked)
        return False
    ...
    def on_retry_failed(self):
        try:
            ...
            hpd.get_python_logger().exception(MSG)
            hpd.show_error("%s: %s" % (MSG, str(e)))

    def on_collect_errors(self):
        try:
            k = hpd.ErrorCollector()
            k.collect_errors(".")
            w = hpd.XHTML(self, k.get_html(), "Errors in '.' and subdirectories")
            w.show()
        except Exception as e:
    ...
    def __update(self):
        ...
        # hpd.get_python_logger().info("UPDATE UPDATE UPDATE UPDATE UPDATE ")
        t = time.time()
        self.__update_status()
        self.__update_table()
        # hpd.get_python_logger().info("&&&&&&&&&&&&&&& time to update: %g" % (time.time()-t,))
    ...
    def __update_status(self):
        ...
        if self.pushButtonStatus.isChecked():
            ...
            self.labelStatusElla.setText(hpd.seconds2str(ella))
            self.labelStatusTPR.setText(hpd.seconds2str(self.rm.time_per_runnable))
            self.labelStatusTotal.setText(hpd.seconds2str(tot))
            self.labelStatusRema.setText(hpd.seconds2str(rema))
    ...
    def __explore_directory(self):
        ...
        if not self.__explorer_form:
            f = self.__explorer_form = hpd.XExplorer(self, dir_)
