...
class WSpectrumCollection(WBase):
    ...
    def __init__(self, parent):
        ...
        lh.addWidget(b)
        ###
        b = self.button_plot_xy = QPushButton("X-Y Plot")
        b.clicked.connect(self.on_plot_xy)
        lh.addWidget(b)
        ###
    ...
    def extract_scalar_clicked(self):
        sspp = self.collection.spectra  # self.get_selected_spectra()
        ...
        if len(sspp) > 0:
            ...
            try:
                ...
                if form.fieldname not in self.collection.fieldnames:
                    self.collection.fieldnames.append(form.fieldname)
                if form.fieldname not in self.collection.fieldnames_visible:
                    self.collection.fieldnames_visible.append(form.fieldname)
