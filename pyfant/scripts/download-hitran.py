...
if __name__ == "__main__":
    ...
    if args.M == _DEF_M:
        print()
        print("\n".join(hpd.format_h1("List of all HITRAN molecules")))
        print()
        # Lists all molecules from local cache of HITRAN molecules
        print(tabulate.tabulate(*hpd.cursor_to_data_header(db.query_molecule())))
        print()
        ...
        if row0 is None:
            hpd.print_error("Molecule ID {} not found".format(args.M))
            sys.exit()

        print()
        print("\n".join(hpd.format_h1("List of all isotopologues for molecule '{Formula}' ({Name})".
                                     format(**row0))))
        print()
        # Lists all molecules from local cache of HITRAN molecules
        print(tabulate.tabulate(*hpd.cursor_to_data_header(
        ...
        if row0 is None:
            ...
            if row1 is None:
                hpd.print_error("Isotopologue ({}, {}) not found".format(args.M, args.I))
                sys.exit()

            print()
            print("\n".join(hpd.format_h1("Isotopologue selected:")))
            print()
            ...
            if args.llzero >= args.llfin:
                hpd.print_error("Initial wavelength must be lower than Final wavelength")
                sys.exit()

            # Here will fetch data
            wn0 = hpd.air_to_vacuum(1e8/args.llzero)
            wn1 = hpd.air_to_vacuum(1e8/args.llfin)
